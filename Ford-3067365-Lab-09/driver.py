...
def main():
	...
	while choice != 6:
		...
		#if a copy has been made, asks the user which tree to use
		if copy:
			if choice == 1 or choice == 2 or choice == 4:
				tree = input('Which tree would you like to use? (original, copy): ')
		...
		#then adds the pokemon to the pokedex
			...
			tree.add(Pokemon(a_name, id, j_name))
		...
		#asks the user for a Pokedex number to be removed, then
		#removes the node in the BST at that Pokedex number
		elif choice == 4:
			try:
				removal = int(input('Please enter the Pokedex number of the Pokemon you would like to remove: '))
		...
		#makes a copy of the pokedex if one is not already created
		elif choice == 5:
			if copy:
